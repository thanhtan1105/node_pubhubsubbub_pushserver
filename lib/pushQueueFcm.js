...
pushQueueFcm.setup = function (_pushKue, _pusher, _db) {
  pushKue = _pushKue
  if (pushKue) {
    pushKue.process(config.pushQueue.queueFcmId, 1, pushQueueFcm._onJob)
  }
...
pushQueueFcm.enqueue = function (deviceType, deviceIds, payload, extraData) {
  ...
  if (_.has(extraData, '_pushQueueFcm__attempted')) {
    if (extraData._pushQueueFcm__attempted >= config.pushQueue.attempts) {
      return false
    }
  ...
  const job = pushKue.create(config.pushQueue.queueFcmId, {
    title: jobTitle,
...
pushQueueFcm._onJob = function (job, jobCallback) {
  ...
  try {
    return pushQueueFcm._onFirebaseJob(job, done)
  } catch (e) {
    debug(e)
