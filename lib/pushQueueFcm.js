...
const pushQueueFcm = exports
const config = require('./config')
const helper = require('./helper')
const debug = require('debug')('pushserver:pushQueueFcm')
const _ = require('lodash')

pushQueueFcm.MESSAGES = {
...
pushQueueFcm.setup = function (_pushKue, _pusher, _db) {
  pushKue = _pushKue
  if (pushKue) {
    pushKue.process(config.pushQueueFcm.queueId, 1, pushQueueFcm._onJob)
  }
...
pushQueueFcm.stats = function () {
  return pusher.stats().then((merged) => {
    merged.pushQueueFcm = _.cloneDeep(stats)
    return merged
  })
}

pushQueueFcm.createQueue = function (kue) {
  const prefix = helper.appendNodeEnv(config.pushQueueFcm.prefix)
...
pushQueueFcm.enqueue = function (deviceType, deviceIds, payload, extraData) {
  ...
  let delay = 0
  if (_.has(extraData, '_pushQueueFcm__attempted')) {
    if (extraData._pushQueueFcm__attempted >= config.pushQueueFcm.attempts) {
      return false
    }

    const powOf2 = Math.pow(2, extraData._pushQueueFcm__attempted - 1)
    delay = config.pushQueueFcm.delayInMs * powOf2
  }

  const job = pushKue.create(config.pushQueueFcm.queueId, {
    title: jobTitle,
  ...
  job.delay(delay)
  job.ttl(config.pushQueueFcm.ttlInMs)
  job.removeOnComplete(true)

  job.save(function (err) {
    if (err) {
      return debug(pushQueueFcm.MESSAGES.QUEUE_ERROR,
...
pushQueueFcm._onJob = function (job, jobCallback) {
  stats.processed++

  const jobDone = helper.later(jobCallback)
  const done = function (err, pusherResult) {
    if (!err) {
      debug(pushQueueFcm.MESSAGES.PUSH_SUCCESS, job.data.title)
      return jobDone()
    }

    debug(pushQueueFcm.MESSAGES.PUSH_ERROR, job.data.title, err, pusherResult)
    ...
    if (_.has(result, 'retries') && result.retries.length > 0) {
      ...
      if (_.has(retryExtraData, '_pushQueueFcm__attempted')) {
        retryExtraData._pushQueueFcm__attempted++
      } else {
        retryExtraData._pushQueueFcm__attempted = 1
      }
  ...
  try {
    pushQueueFcm._onFirebaseJob(job, done)
    return done(pushQueueFcm.MESSAGES.JOB_ERROR_UNRECOGNIZED_DEVICE_TYPE)
  } catch (e) {
    debug(e)
    return done(pushQueueFcm.MESSAGES.JOB_ERROR_PUSHER)
  }
}

pushQueueFcm._onFirebaseJob = function (job, callback) {
  const done = helper.later(callback)

  const { data } = job
  const { extra_data: extraData } = data
  if (typeof extraData !== 'object') {
    return done(pushQueueFcm.MESSAGES.JOB_ERROR_PROJECT_EXTRA_DATA_MISSING,
  ...
  db.projects.findConfig('fcm', projectId, (config) => {
    if (!config) {
      ...
      return done(pushQueueFcm.MESSAGES.JOB_ERROR_PROJECT_NOT_FOUND,
        { invalids: data.device_ids })
    }
...
pushQueueFcm._reset = function (callback) {
